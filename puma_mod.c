...
// ** pp. 109-119, 2003.                                           **
// **                                                              **
// ** S. Ventura, E. G. Birgin, J. M. Mart�nez and                 **
...
// **                                                              **
// ** R. Andrade, E. G. Birgin, I. Chambouleyron, J. M. Mart�nez   **
	   ...
	   double z[][nobsmax],double n[][nobsmax])
{
  ...
  // DEBUG MÍNIMO - solo primeros y últimos valores
  printf("=== DEBUG compn ===\n");
  printf("h = %f, nobs = %d\n", h, nobs);
  ...
  for ( i = 1; i <= slayer-1; i ++ )
    {
      n[i][nobs-1] = fmax(1.0, 1 + z[i-1][nobs-1] * z[i-1][nobs-1]);
      n[i][nobs-2] = fmax(1.0, n[i][nobs-1] + h * z[i-1][nobs-2] * z[i-1][nobs-2]);

      for ( j = nobs-3; j >= 0; j -- )
	      n[i][j] = fmax(1.0, 2 * n[i][j+1] - n[i][j+2] + h * h * z[i-1][j] * z[i-1][j]);
      // SOLO 3 PUNTOS CRÍTICOS
      printf("Capa %d: n[0]=%.6f, n[%d]=%.6f, n[%d]=%.6f\n", 
             i, n[i][0], nobs/2, n[i][nobs/2], nobs-1, n[i][nobs-1]);
    }

  for ( i = slayer; i <= nlayers-3; i ++ )
    {
      n[i+1][nobs-1] = fmax(1.0, 1 + z[i-1][nobs-1] * z[i-1][nobs-1]);
      n[i+1][nobs-2] = fmax(1.0, n[i+1][nobs-1] + h * z[i-1][nobs-2] * z[i-1][nobs-2]);

      for ( j = nobs-3; j >= 0; j -- )
	n[i+1][j] = fmax(1.0, 2 * n[i+1][j+1] - n[i+1][j+2] + h * h * z[i-1][j] * z[i-1][j]);
    }
}
	    ...
	    double z[][nobsmax],double n[][nobsmax], double dndz[][nobsmax])
{
  ...
  // Films refractive indices with constraints
  for ( i = 1; i <= slayer-1; i ++ )
    {
      n[i][nobs-1] = fmax(1.0, 1 + z[i-1][nobs-1] * z[i-1][nobs-1]);
      n[i][nobs-2] = fmax(1.0, n[i][nobs-1] + h * z[i-1][nobs-2] * z[i-1][nobs-2]);
	   ...
	   double ip[],double lambda[],double v[][nobsmax],double k[][nobsmax])
{
  ...
  // Films absorption coefficient with constraints
  for ( i = 1; i <= slayer-1; i ++ )
    {
      ipind = 0;
	...
	ipind ++;
      ...
      k[i][nobs-1] = fmax(0.0, v[i-1][nobs-1] * v[i-1][nobs-1]);
      k[i][nobs-2] = fmax(0.0, k[i][nobs-1] + h * v[i-1][nobs-2] * v[i-1][nobs-2]);

      for ( j = nobs-3; j >= ipind; j -- )
	k[i][j] = fmax(0.0, 2 * k[i][j+1] - k[i][j+2] + h * h * v[i-1][j] * v[i-1][j]);
  ...
  for ( i = slayer; i <= nlayers-3; i ++ )
	...
	ipind ++;
      ...
      k[i+1][nobs-1] = fmax(0.0, v[i-1][nobs-1] * v[i-1][nobs-1]);
      k[i+1][nobs-2] = fmax(0.0, k[i+1][nobs-1] + h * v[i-1][nobs-2] * v[i-1][nobs-2]);

      for ( j = nobs-3; j >= ipind; j -- )
	k[i+1][j] = fmax(0.0, 2 * k[i+1][j+1] - k[i+1][j+2] + h * h * v[i-1][j] * v[i-1][j]);
...
double evalf(int ydim,double y[])
{
  ...
  // DEBUG: Verificar n después del cálculo
  printf("=== n después de compn ===\n");
  for (i = 1; i <= nfilms; i++) {
      printf("Film %d: n_min=%.6f, n_max=%.6f\n", 
             i, n[i][0], n[i][nobs-1]);
  }
  compk(nlayers,slayer,nobs,ip,lambda,v,k);

  // Penalización por violación de restricciones físicas
  double penalty = 0.0;
  for ( i = 1; i <= slayer-1; i ++ )
    for ( j = 0; j < nobs; j ++ ) {
      if (n[i][j] < 1.0) 
        penalty += 1e4 * (1.0 - n[i][j]);
      if (k[i][j] < 0.0)
        penalty += 1e4 * (-k[i][j]);
    }
             ...
             int nobs,double lini,double lfin,char datfile[])
{
      ...
      for ( i = 0; i < nobs; i ++ )
        {
          ...
          /* Sellmeier coefficients (B270-type SLG approximation) */
          double B1 = 1.03961212;
          double C1 = 0.00600069867;
          ...
          /* Compute terms */
          double term1 = B1 * lam2 / (lam2 - C1);
          double term2 = B2 * lam2 / (lam2 - C2);
          double term3 = B3 * lam2 / (lam2 - C3);

          double n2 = 1.0 + term1 + term2 + term3;

          /* Safety: avoid nonphysical values */
          if ( n2 <= 0.0 )
            {
              sprintf(message,
                      "Error: n^2 <= 0 for substrate 70 at lambda=%g nm (n2=%g)",
                      film.lambda[i], n2);
              Error(message);
            }
  ...
  // Invalid substrate code
  else
      Error("Invalid substrate (10=Glass, 20=Crystalline silicon, 30=Crystalline quartz, 40=Glass slides, 50=Borosilicate, 60=Amorphous quartz, 70=SLG)");
...
void IniPoint(double n0[],double nf[],double k0[],double y[])
{
  ...
  for ( i = 0; i < nfilms; i ++ )
    ...
    k1 = fmax(0.0, 0.1 * k0[i]);        // Añadí fmax aquí
    k2 = fmax(0.0, 1e-10 * k0[i]);      // Añadí fmax aquí

    for ( j = 0; j < nobs; j++ )
      {
        n[j] = fmax(1.0, n0[i] - ( n0[i] - nf[i] ) * j / ( nobs - 1 ));
        if ( j < nk1 ) 
          k[j] = fmax(0.0, k0[i] - ( k0[i] - k1 ) * ( j - nk0 ) / ( nk1 - nk0 ));
        else 
          k[j] = fmax(0.0, k1    - ( k1    - k2 ) * ( j + 1 - nk1 ) / ( nk2 - nk1 ));
      }

    z[nobs-1] = sqrt( fmax(0.0, n[nobs-1] - 1 ) );                    // Cambié max por fmax
    z[nobs-2] = sqrt( fmax(0.0, ( n[nobs-2] - n[nobs-1] ) / h ) );    // Cambié max por fmax

    v[nobs-1] = sqrt( fmax(0.0, k[nobs-1] ) );                        // Cambié max por fmax
    v[nobs-2] = sqrt( fmax(0.0, ( k[nobs-2] - k[nobs-1] ) / h ) );    // Cambié max por fmax

    for ( j = 0; j < nobs - 2; j ++ )
      {
      z[j] = sqrt( fmax(0.0, ( n[j] - 2 * n[j+1] + n[j+2] ) / ( h * h ) ) );  // Cambié max por fmax
      v[j] = sqrt( fmax(0.0, ( k[j] - 2 * k[j+1] + k[j+2] ) / ( h * h ) ) );  // Cambié max por fmax
      }
